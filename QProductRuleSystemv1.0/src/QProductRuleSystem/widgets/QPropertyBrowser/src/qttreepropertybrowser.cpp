...
void QtPropertyEditorView::mousePressEvent(QMouseEvent *event)
{
    ...
    (item->flags().testFlag(Qt::ItemIsEnabled) || item->flags().testFlag(Qt::ItemIsEditable));

    if (item) {
        if ((item != m_editorPrivate->editedItem()) && (event->button() == Qt::LeftButton)
                && (header()->logicalIndexAt(event->pos().x()) == 1)
                && (item->flags().testFlag(Qt::ItemIsEnabled) || item->flags().testFlag(Qt::ItemIsEditable))) { // ((item->flags() & (Qt::ItemIsEditable | Qt::ItemIsEnabled)) == (Qt::ItemIsEditable | Qt::ItemIsEnabled))
...
void QtTreePropertyBrowserPrivate::init(QWidget *parent)
{
    ...
    QStringList labels;
    labels.append(QCoreApplication::translate("QtTreePropertyBrowser", "Свойства")); // "Property"
    labels.append(QCoreApplication::translate("QtTreePropertyBrowser", "Значения")); // "Value"
