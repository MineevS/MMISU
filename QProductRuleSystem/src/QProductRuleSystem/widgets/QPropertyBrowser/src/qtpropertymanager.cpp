...
#else
    void indexToLocale(int languageIndex, int territoryIndex, QLocale::Language *language, QLocale::Country *territory) const;
    void localeToIndex(QLocale::Language language, QLocale::Country territory, int *languageIndex, int *territoryIndex) const;
#endif
private:
    void initLocale();
...
#else
    QMap<int, QMap<int, QLocale::Country> > m_indexToTerritory;
    QMap<QLocale::Language, QMap<QLocale::Country, int> > m_territoryToIndex;
#endif
...
static QList<QLocale::Country> sortedTerritories(const QList<QLocale> &locales)
{
    QMultiMap<QString, QLocale::Country> nameToTerritory;
    for (const QLocale &locale : locales) {
        const auto territory = locale.country();
        nameToTerritory.insert(QLocale::countryToString(territory), territory);
    }
    return nameToTerritory.values();
}
#endif
...
#else
            for (QLocale::Country territory : territories) {
                territoryNames << QLocale::countryToString(territory);
                m_indexToTerritory[langIdx][territoryIdx] = territory;
                m_territoryToIndex[language][territory] = territoryIdx;
                ++territoryIdx;
            }
...
#else
void QtMetaEnumProvider::indexToLocale(int languageIndex, int territoryIndex, QLocale::Language *language, QLocale::Country *territory) const
{
    QLocale::Language l = QLocale::C;
#if QT_VERSION >= 0x051300
        QLocale::Territory c = QLocale::AnyTerritory;
...
void QtMetaEnumProvider::localeToIndex(QLocale::Language language, QLocale::Country territory, int *languageIndex, int *territoryIndex) const
{
    int l = -1;
    int c = -1;
    if (m_languageToIndex.contains(language)) {
...
#else
    QLocale::Country newTerritory = loc.country();
#endif
...
#else
    me->localeToIndex(loc.language(), loc.country(), &langIdx, &territoryIdx);
#endif
...
#else
    qWarning("QtLocalePropertyManager::valueText: Unknown territory %d for %s", loc.country(), qPrintable(languageName));
#endif
        return languageName;
    }
...
#else
    metaEnumProvider()->localeToIndex(val.language(), val.country(), &langIdx, &territoryIdx);
#endif
...
void QtFlagPropertyManager::setFlagNames(QtProperty *property, const QStringList &flagNames)
{
    ...
    if (pfit != d_ptr->m_propertyToFlags.end()) {
        for (QtProperty *prop : pfit.value()) {//std::as_const(pfit.value())
            if (prop) {
                delete prop;
...
void QtFlagPropertyManager::uninitializeProperty(QtProperty *property)
{
    ...
    if (it != d_ptr->m_propertyToFlags.end()) {
        for (QtProperty *prop : it.value())  { //std::as_const(it.value())
            if (prop) {
                d_ptr->m_flagToProperty.remove(prop);
                delete prop;
            }
